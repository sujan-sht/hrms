...
@section('breadcrum')
    <a class="breadcrumb-item active"> TADA / Claims </a>
@endsection
@section('script')
    ...
    <div class="card card-body">
        <div class="media align-items-center align-items-md-start flex-column flex-md-row">
            ...
            <div class="media-body text-center text-md-left">
                <h6 class="media-title font-weight-semibold">List of Claims</h6>
                ...
                <table class="table table-hover">
                    <thead>
                        <tr class="text-light btn-slate">
                            ...
                            <th>Status</th>
                            @if ($menuRoles->assignedRoles('tada.edit') ||
                            ...
                            @foreach ($tadas as $key => $tada)
                                @php
                                    ...
                                    if ($usertype == 'admin' || $usertype == 'super_admin' || $usertype == 'hr'|| $usertype == 'division_hr') {
                                        ...
                                        if (($thisStatus == 'Forwarded' || $thisStatus == 'Partially Settled') || ($singleApproval == true && $thisStatus == 'Pending')) {
                                            $showUpdate = true;
                                        } else {
                                            $showUpdate = false;
                                        }
                                ...
                                <tr>
                                    ...
                                    <td>{{ optional($tada->employee)->first_name }}
                                        {{ optional($tada->employee)->last_name }}
                                    </td>
                                    @if (setting('calendar_type') == 'BS')
                                    ...
                                    @if ($menuRoles->assignedRoles('tada.edit') ||
                                        $menuRoles->assignedRoles('tada.delete') ||
                                        $menuRoles->assignedRoles('tada.show') ||
                                        $menuRoles->assignedRoles('tada.downloadPdfClaim') ||
                                        $menuRoles->assignedRoles('tada.updateStatus'))
                                        <td class="d-flex">
                                            ...
                                            @endif
                                            @if ($menuRoles->assignedRoles('tada.downloadPdfClaim'))
                                                <a class="btn btn-outline-warning btn-icon mx-1"
                                                    href="{{ route('tada.downloadPdfClaim', $tada->id) }}" data-popup="tooltip"
                                                    data-placement="bottom" data-original-title="Download PDF"><i
                                                        class="icon-download"></i>
                                                </a>
                                            @endif
                                            @if($menuRoles->assignedRoles('tada.updateStatus') && ($tada->status == '1' || $tada->status == '2' || $tada->status == '6') && $showUpdate)
