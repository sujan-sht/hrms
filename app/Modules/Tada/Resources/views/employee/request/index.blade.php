...
@section('title')Request Advance @stop
@section('breadcrum')Request Advance @stop
@section('scripts')
            ...
            <div class="col-12">
                <div class="card mb-2">
                    <div class="card-header">
                        <h6 class="float-left">Advance Request Management</h6>
                        @if ($menuRoles->assignedRoles('employeetadaRequest.create'))
                            <a href="{{ route('employeetadaRequest.create') }}" class="btn btn-primary float-right text-white"
                                type="button"><i class="icon-plus2"></i> Add
                                Request</a>
                        @endif
                    </div>
                    <div class="card-body table-content">
                        <table class="table">
                            <tr class="bg-slate">
                                ...
                                @if (
                                    $menuRoles->assignedRoles('employeetadaRequest.edit') ||
                                        $menuRoles->assignedRoles('employeetadaRequest.delete') ||
                                        $menuRoles->assignedRoles('employeetadaRequest.show') ||
                                        $menuRoles->assignedRoles('employeetadaRequest.edit'))
                            ...
                            @if ($tadas->total() > 0)
                                @foreach ($tadas as $tada)
                                    <tr>
                                        ...
                                        <!-- @if ($menuRoles->assignedRoles('employeetadaRequest.show'))
    -->
                                        <td>
                                            @if ($menuRoles->assignedRoles('employeetadaRequest.show'))
                                                <a class="mr-3" href="{{ route('employeetadaRequest.show', $tada->id) }}"
                                                    data-original-title="History">
                                                    <i class="icon-eye"></i> </a>
                                            @endif
                                            @if ($menuRoles->assignedRoles('employeetadaRequest.edit'))
                                                <a class="mr-3" href="{{ route('employeetadaRequest.edit', $tada->id) }}"
                                                    data-original-title="History" title="Edit">
                                                    <i class="fa fa-edit"></i></a>
                                            @endif
                                            @if ($menuRoles->assignedRoles('employeetadaRequest.delete') && $tada->status == 'pending')
                                                <a class="delete delete_tada" data-toggle="modal"
                                                    data-target="#modal_theme_warning"
                                                    link="{{ route('employeetadaRequest.delete', $tada->id) }}"
