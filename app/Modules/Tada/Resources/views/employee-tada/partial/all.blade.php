    ...
    @if ($tadas->total() != 0)
        @foreach ($tadas as $key => $value)
            <tr>
                ...
                <td>{{ optional($value->employee)->first_name . ' ' . optional($value->employee)->middle_name . ' ' . optional($value->employee)->last_name }}
                </td>
                <td class="text-teal">
                ...
                <td>
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="actionMenu">
                            @if ($value->first_approval_id == Auth::user()->id && $value->status == 'pending')
                                <a class="dropdown-item update_request" data-toggle="modal"
                                    data-target="#modal_theme_warning"
                                    link="{{ route('team-tadaRequest.update', $value->id) }}"
                                    att-status="{{ $value->status }}" data-placement="bottom" data-popup="tooltip"
                                    data-original-title="Update Status">
                                    <i class="fa fa-edit"></i> Update Status</a>
                            @endif
                            @if ($value->forwarded_to == Auth::user()->id)
                                <a class="dropdown-item update_other_request" data-toggle="modal"
                                    data-target="#modal_theme_update"
                                    link="{{ route('team-tadaRequest.update', $value->id) }}"
                                    att-status="{{ $value->status }}" data-placement="bottom" data-popup="tooltip"
                                    data-original-title="Update Status">
                                    <i class="fa fa-edit"></i> Update Status</a>
