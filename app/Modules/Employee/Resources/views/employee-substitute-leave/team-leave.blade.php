        ...
        <table class="table table-hover">
            ...
            <thead>
                <tr class="text-light btn-slate">
                    ...
                    <th>Remark</th>
                    <th>Status</th>
                    @if ($menuRoles->assignedRoles('substituteLeave.updateStatus'))
                            ...
                            if ($emp_id == $teamLeaveModel->employee_id) {
                                ...
                                if (!empty($emp_leave_flow)) {
                                    if (!empty($emp_leave_flow->first_approval_user_id) && $emp_leave_flow->first_approval_user_id > 0) {
                                        if ($teamLeaveModel->status == 1 && $emp_leave_flow->first_approval_user_id == $user_id) {
                                            $emp_status = 5; //pending, rejected, forwarded
                                        } elseif ($teamLeaveModel->status == 2 && $emp_leave_flow->first_approval_user_id == $user_id) {
                                            $emp_status = 2; //forwarded
                                        } elseif ($teamLeaveModel->status == 4 && $emp_leave_flow->first_approval_user_id == $user_id) {
                                            $emp_status = 4; //rejected
                                        } elseif ($teamLeaveModel->status == 2 && $emp_leave_flow->last_approval_user_id == $user_id) {
                                            $emp_status = 7; //forwarded, rejected, accepted
                                        } elseif ($teamLeaveModel->status == 1 && $emp_leave_flow->last_approval_user_id == $user_id) {
                                            $emp_status = 6; //pending
                                        } elseif ($teamLeaveModel->status == 3 && $emp_leave_flow->first_approval_user_id == $user_id) {
                                            $emp_status = 3; //empty
                                        } elseif ($teamLeaveModel->status != 1 && $teamLeaveModel->status != 2 && $emp_leave_flow->last_approval_user_id == $user_id) {
                                            $emp_status = 3; //empty
                                        }
                                    } else {
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="media">
                                    <div class="mr-3">
                                        <a href="#">
                                            <img src="{{ optional($teamLeaveModel->employee)->getImage() }}"
                                                class="rounded-circle" width="40" height="40" alt="">
                                        </a>
                                    </div>
                                    <div class="media-body">
                                        <div class="media-title font-weight-semibold">
                                            {{ optional($teamLeaveModel->employee)->getFullName() }}</div>
                                        <span
                                            class="text-muted">{{ optional($teamLeaveModel->employee)->official_email }}</span>
                                    </div>
                                </div>
                            </td>
                            <td>{{ date('M d,Y', strtotime($teamLeaveModel->date)) }}</td>
                            <td>1</td>
                            <td>{!! $teamLeaveModel->remark !!}</td>
                            <td>
                                <span class="badge badge-{{ $teamLeaveModel->getStatusWithColor()['color'] }}">{{ $teamLeaveModel->getStatusWithColor()['status'] }}</span>
                            </td>
                            <td>
                                @if (auth()->user()->user_type == 'super_admin' || ($menuRoles->assignedRoles('substituteLeave.updateStatus') && in_array($teamLeaveModel->status, [1,2])))
                                    <a data-toggle="modal" data-target="#updateStatus" class="btn btn-outline-warning btn-icon updateStatus mr-1" data-id="{{ $teamLeaveModel->id }}"
                                        data-status="{{ $teamLeaveModel->status }}" emp_status="{{ $emp_status }}" data-popup="tooltip" data-placement="top" data-original-title="Status">
