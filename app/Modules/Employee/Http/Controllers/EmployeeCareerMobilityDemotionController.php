...
use App\Modules\Employee\Services\EmployeeCareerMobilityDemotionService;

class EmployeeCareerMobilityDemotionController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (!empty($filter)) {
            ...
            $data['employeeCareerMobilityDemotion'] = EmployeeCarrierMobilityDemotion::where('employee_id', $filter['employee_id'])->first();
        ...
        return view('employee::employee.carrier-mobility.demotion.index', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $employeeCareerMobilityDemotionService = new EmployeeCareerMobilityDemotionService($request);

            $data = $employeeCareerMobilityDemotionService->setDemotionData();
            $employeeCarrierMobilityDemotion =  EmployeeCarrierMobilityDemotion::create($data);

            $timelineData = $employeeCareerMobilityDemotionService->setTimeLineData($employee, $employeeCarrierMobilityDemotion);
            NewEmployeeCareerMobilityTimeline::create($timelineData);

            $timelineData['date'] = now();
            $timelineData['reference'] = 'EmployeeCarrierMobilityDemotion';
            ...
            $employeeCareerMobilityDemotionService->updateEmployeeDetails($employee);
