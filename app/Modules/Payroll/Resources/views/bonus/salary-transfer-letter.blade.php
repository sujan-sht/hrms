                ...
                <div class="col-10">
                    <div class="salary-sheet-body">
                        <div class="salary-paid-block">
                            @if($bonusModel->calendar_type == 'nep')
                            @php $month = date_converter()->_get_nepali_month($bonusModel->month) @endphp
                            @else
                            @php $month = date_converter()->_get_english_month($bonusModel->month) @endphp
                            @endif
                            <h4>Employee Salary Transfer Letter of
                                {{-- @if($bonusModel->calendar_type == 'eng') {{date('F - Y', strtotime($bonusModel->year.'-'.$bonusModel->month))}}@elseif($calender_type == 'nep') {{ ($bonusModel->month.' - '.$bonusModel->month) }}@endif --}}
                                {{ ($bonusModel->year.' - ' . $month) }}
                            </h4>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-12">
                    <div class="salary-sheet-header">
                        <div class="company-name">
                            ...
                            <span>{{ optional($bonusModel->organization)->bank_name }}</span><br />
                            <span>Nepal</span>
                        </div>
                ...
                <div class="col-12">
                    <div class="salary-sheet-body">
                        <div class="salary-paid-block">
                            ...
                            <p>On behalf of {{ optional($bonusModel->organization)->name }}, I request
                            you to please debit salary of all our employees from our bank A/C No. {{ $bonusModel->organization->bank_account_no }}, the details of which have been mentioned below: </p>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="table table-striped" id="table2excel">
                        ...
                        <tbody class="bg-slate-400 text-white">
                            @php
                                $num = 0;
                            @endphp
                            @if(count($bonusModel->bonusEmployees) > 0)
                                        ...
                                        <td>
                                            <div class="media">
                                                <div class="media-body">
                                                    <div class="media-title font-weight-semibold">{{ optional($bonusEmployee->employee)->getFullName() }}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td>{{optional(optional(optional($bonusEmployee->employee)->bankDetail)->bankinfo)->dropvalue}}</td>
                                        <td>{{optional(optional($bonusEmployee->employee)->bankDetail)->bank_code}}</td>
                                        <td>{{optional(optional($bonusEmployee->employee)->bankDetail)->account_number}}</td>
                                        <td>{{$bonusEmployee->payable_salary}}</td>
                                        {{-- <td>{{$bonusEmployee->remarks}}</td> --}}
                                    </tr>
                                @endforeach
                    ...
                    <div class="salary-sheet-footer">
                        <div class="salary-sheet-body">
                            <div class="salary-paid-block">
                                ...
                                <span>{{ optional($bonusModel->organization)->name }}</span>
                            </div>
                        </div>
