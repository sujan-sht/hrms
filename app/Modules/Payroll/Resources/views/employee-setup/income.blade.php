...
@section('content')
    ...
    <div class="text-right mb-4 mr-5">
        <a href="{{ route('payroll.exportIncome', request()->all()) }}"
    ...
    @include('payroll::employee-setup.partial.income-setup-upload')
        @if (count($incomeList) > 0)
            {!! Form::open([
                'route' => 'employeeSetup.store.income',
                'method' => 'POST',
                'id' => '',
                'class' => 'form-horizontal',
                'role' => 'form',
            ]) !!}
            @include('payroll::employee-setup.partial.income-create', ['btnType' => 'Save Record'])
