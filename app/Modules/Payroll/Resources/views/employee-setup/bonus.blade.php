...
@section('content')
    @include('payroll::employee-setup.partial.bonus_search')
    @if (request()->get('organization_id'))
    <div class="text-right mb-4 mr-5">
        <a href="{{ route('payroll.exportBonus', request()->all()) }}"
    ...
    @include('payroll::employee-setup.partial.bonus-setup-upload')
        @if (count($bonusList) > 0)
            {!! Form::open([
                'route' => 'employeeSetup.store.bonus',
                'method' => 'POST',
                'id' => '',
                'class' => 'form-horizontal',
                'role' => 'form',
            ]) !!}
            @include('payroll::employee-setup.partial.bonus-create', ['btnType' => 'Save Record'])
