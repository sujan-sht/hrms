            ...
            <table class="table table-responsive table-bordered" id="table2excel">
                ...
                <tbody>
                    @if (count($payrollModel->payrollEmployees) > 0)
                        ...
                        {{-- @if($payrollEmployee->employee_id == 346) --}}
                            @php
                                $totalDeduction = 0;
                                $totalIncome = 0;
                                $total_days = 0;
                            ...
                            @if ($terminatedDate && ($payrollModel->year == $terminatedYear && $payrollModel->month == $terminatedMonth))
                                <tr class="myLine">
                                    ...
                                    <td>{{ optional(optional($payrollEmployee->employee)->branchModel)->name }}</td>
                                    <td>{{ optional(optional($payrollEmployee->employee)->designation)->dropvalue }}</td>
                                    <td>{{ $joinDate }}</td>
                                    ...
                                    @php
                                        ...
                                        $payable_days = $attendance['total_days'] - $payrollEmployee->unpaid_leave_days;
                                        // dd($total_leave);
                                    @endphp
                                    @if ($terminatedDate && ($payrollModel->year == $terminatedYear && $payrollModel->month == $terminatedMonth))
                                        <td>{{ $terminatedDay }}</td>
                                    ...
                                    <td>
                                        @php
                                            $taxableAmount = $payrollEmployee->yearly_taxable_salary ? $payrollEmployee->yearly_taxable_salary : 0;
                                    ...
                                    <td>
                                        @php
                                            $advance = isset($payrollEmployee->advanceAmount) ? $payrollEmployee->advanceAmount : 0;
