    ...
    <div class="card card-body">
        <div class="media align-items-center align-items-md-start flex-column flex-md-row">
            ...
            <div class="media-body text-center text-md-left">
                <h6 class="media-title font-weight-semibold">List of Travel Requests</h6>
                All the Travel Requests Information will be listed below. You can Create and Modify the data.
            </div>
                    ...
                    @if ($businessTrips->total() > 0)
                        @foreach ($businessTrips as $key => $businessTrip)
                            <tr>
                                    ...
                                    <td>
                                        <div class="media">
                                            ...
                                            <div class="media-body">
                                                ...
                                                <span
                                                    class="text-muted">{{ optional($businessTrip->employee)->official_email ?? optional($businessTrip->employee)->personal_email }}
                                                </span>
                                                 <span
                                                    class="text-muted">{{ $businessTrip->designation }}
                                                </span>
                                            </div>
                                        </div>
                                    </td>
                                ...
                                <td>{{ $travelTypes[$businessTrip->type_id ?? ''] }}</td>
                                ...
                                <td>{{ $businessTrip->advance_amount }}</td>
                                <td>{{ $businessTrip->transportType->title ?? '' }}</td>
                                <td>{{ $businessTrip->purpose }}</td>
                                <td>@if($businessTrip->document )<a href="{{ $businessTrip->document }}" target="_blank">File </a> @endif</td>
                                ...
                                {{-- @if ($businessTrip->claim_status == 2 && $businessTrip->employee_id != optional(auth()->user()->userEmployer)->id)
                                    <td>Rs. {{ $businessTrip->eligible_amount ? $businessTrip->eligible_amount :0 }}</td>
                                @endif --}}

                                <td class="text-center">
                                    <span
                                        class="badge badge-{{ $colors[$businessTrip->getStatus() ?? 'Pending'] }}">{{ $businessTrip->getStatus() ?? 'Pending' }}</span>
                                </td>
                                <td class="text-center">
                                    <span
                                        class="badge badge-{{ $colors[$businessTrip->getClaimStatus() ?? 'Pending'] }}">{{ $businessTrip->getClaimStatus() ?? 'Pending' }}</span>
                                </td>
                                <td>{{ $businessTrip->claim_status == 2 ? $businessTrip->eligible_amount : '-' }}</td>

                                <td class="text-center d-flex">
                                    @if (
                                        $menuRoles->assignedRoles('businessTrip.updateClaimStatus') &&
                                            $businessTrip->status == 3 &&
                                            $businessTrip->claim_status == 1 &&
                                        ...
                                        <a class="btn btn-outline-info btn-icon mx-1"
                                            ...
                                            data-placement="top" data-original-title="Claim">
                                    ...
                                    @if ($menuRoles->assignedRoles('businessTrip.edit') && $businessTrip->status == 1)
                                        <a class="btn btn-outline-primary btn-icon mx-1"
                                            href="{{ route('businessTrip.edit', $businessTrip->id) }}" data-popup="tooltip"
                                    ...
                                    @endif
                                    @if ($menuRoles->assignedRoles('businessTrip.downloadPDF'))
                                        <a class="btn btn-outline-primary btn-icon mx-1"
                                    ...
                                    @if (
                                        $menuRoles->assignedRoles('businessTrip.cancelRequest') &&
                                            $businessTrip->status == 1 &&
                                            $businessTrip->employee_id == Auth::user()->emp_id)
                                        {!! Form::open([
                                            'route' => 'businessTrip.cancelRequest',
                                            'method' => 'PUT',
                                        ...
                                        <button class="btn btn-outline-warning btn-icon mr-1 confirmCancel"
                                            ...
                                            link="{{ route('businessTrip.cancelRequest', $businessTrip->id) }}">
                                    ...
                                    @if ($menuRoles->assignedRoles('businessTrip.delete') && ($businessTrip->status == 1 || $businessTrip->status == 4))
