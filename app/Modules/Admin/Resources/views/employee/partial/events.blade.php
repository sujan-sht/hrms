    ...
    <div class="card-header bg-transparent header-elements-inline">
        ...
        <div class="header-elements">
            <div class="list-icons ml-3">
                <a class="btn btn-success btn-sm rounded-pill" href="{{ route('dashboard.calendar')}}">
                    View More
                </a>
            </div>
        </div>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        <div class="mr-3">
                                            ...
                                            @endphp
                                            <a href="#" class="btn btn-{{ $colorClass }} rounded-pill btn-icon btn-sm">
                                                <span class="">{{ $alphabet }}</span>
                                            </a>
                                        </div>
                                        <div>
                                            <a href="{{ route('dashboard.calendar',['start_date'=>$holiday_val['date']]) }}" class="text-body font-weight-semibold letter-icon-title">{{ $holiday_val['title'] }}</a>
                                            <div class="text-muted font-size-sm"><i class="icon-calendar2 font-size-sm mr-1"></i>{{ getStandardDateFormat($holiday_val['date']) }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <h6 class="font-weight-semibold mb-0 text-right">
                                        @if (date('Y m d') == date('Y m d', strtotime($holiday_val['date'])))
                                            <div class="text-danger">Today</div>
                                        @elseif(date('Y m d', strtotime($holiday_val['date'])) < date('Y m d') )
                                            {{ Carbon\Carbon::parse(Carbon\Carbon::now())->diffInDays($holiday_val['date']) }}
                                            Day(s) Ago
                                        @else
