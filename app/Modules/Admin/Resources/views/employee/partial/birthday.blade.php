<div class="card " style="height: 348px;">
    ...
    <div class="card-body table-responsive" style="overflow-y: auto;overflow-x: hidden; padding:0">

        @if (count($birthdayAnniversary) > 0)

            {{-- <table class="table text-nowrap">
                <tbody>
                    @foreach ($birthdayAnniversary as $model)
                        <tr>
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="mr-3">
                                        <a href="" class="btn rounded-pill btn-icon btn-sm">
                                            <img src="{{ $model->getImage() }}" class="rounded-circle" width="40"
                                                height="40" alt="">
                                        </a>
                                    </div>
                                    <div>
                                        <a href=""
                                            class="text-body font-weight-semibold letter-icon-title">{{ $model->getFullName() }}</a>
                                        <div class="text-muted font-size-sm"><i
                                                class="icon-calendar2 font-size-sm mr-1"></i>{{ getStandardDateFormat($model['date']) }}
                            ...
                            <td>
                                <h6 class="font-weight-semibold mb-0 text-right">
                                    ...
                                    @endphp
                                    <span class="badge badge-{{ $badge }}">{{ ucfirst($model['type']) }}</span>
                                        ...
                                        <div class="media-body">
                                            ...
                                            <div>
                                                <div>
                                                    ...
                                                    <a href="{{ route('tool.sendWishMail', $model->id) }}"
                                                        class="mt-1 btn btn-outline-secondary btn-sm send-wish-btn"> <i
                                                            class="icon-mail5"></i> Send Wish</a>
                                                </div>
                                                @if (!is_null($model->phone))
                                                    <div>
                                                        <span class="text-muted">
                                                            {{ $model->user->getOtpPhone() }}
                                                        </span>
                                                        <a href="{{ route('tool.sendWishSMS', ['id' => $model->id, 'type' => \Illuminate\Support\Str::slug($model['type'])]) }}"
                                                            class="mt-1 btn btn-outline-secondary btn-sm send-wish-btn"> <i
                                                                class="icon-phone"></i> Send Wish</a>
                                                    </div>
                                                @endif
                                    ...
                                    <div class="row" style="padding: 5px 0px;">
                                        <div class="col-md-12 text-center">
                                            <h6 class="font-weight-semibold mb-0">
                                                ...
                                                @endphp
                                                <span
                                                    class="badge badge-{{ $badge }}">{{ ucfirst($model['type']) }}</span>
...
<script>
$(document).on('click', '.send-wish-btn', function (e) {
    var $btn = $(this);
