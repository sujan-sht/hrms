@inject('pollRepo', '\App\Modules\Poll\Repositories\PollRepository')

<div class="card" style="height: 97%;">
    <div class="card-header bg-secondary header-elements-inline text-white">
        ...
        <div class="header-elements">
            <div class="list-icons ml-3">
                <a href ="{{route('poll.viewEmployeeReport', ['status'=>'active'])}}" class="badge badge-primary badge-pill">View All</a>
            </div>
        </div>
    </div>

    <div class="card-body table-responsive" style="overflow-x: hidden; padding:0">
        @if (isset($pollModel) && !empty($pollModel))
            @php
                $totalResponses = count($pollModel->responses);
                $isVoted = 'no';
                $pollResponse = $pollRepo->checkResponseSubmitted($pollModel->id, optional(auth()->user()->userEmployer)->id);
                if(isset($pollResponse) && !empty($pollResponse)){
                    $isVoted = 'yes';
                }
            @endphp
            <table class="table text-nowrap">
                <tbody>
                    <tr>
                        <td>
                            <div>
                                <h6 class="mb-2">{{ $pollModel->question }}</h6>

                                {!! Form::hidden('poll_id', $pollModel->id, ['id' => 'poll']) !!}
                                @foreach ($pollModel->options as $option)
                                    <div class="col-lg-12 mb-2">
                                        <div class="row">
                                            <div class="col-lg-11 rounded">
                                                <div class="d-flex flex-column">
                                                    @php
                                                        ...
                                                        $color = '';
                                                        $type = 'progress';
                                                        if (isset($option->getEmployeePollResponse(optional(auth()->user()->userEmployer)->id)->first()['poll_option_id'])) {
                                                            if ($option->getEmployeePollResponse(optional(auth()->user()->userEmployer)->id)->first()['poll_option_id'] == $option->id) {
                                                                $checked = 'checked';
                                                                $color = 'teal';
                                                                $type = 'progress-bar';
                                                            }
                                                    ...
                                                    <div class="radioBtn">
                                                        @if ($isVoted == 'no')
                                                            <input type="radio" name="poll_option_id"
                                                                value="{{ $option->id }}" class="pollOption"
                                                                {{ $checked }}>
                                                        @endif
                                                        <label class="ml-2"
                                            ...
                                            <div class="col-md-1">
                                                @if ($pollModel->show_on_employee == 11)
                                                    @php
                                                        $optionWiseResponses = count($option->responses);
                                                        $responsePercentage = 0;
                                                        if($optionWiseResponses > 0){
                                                            $responsePercentage = number_format(($optionWiseResponses / $totalResponses)*100, 2);
                                                        }
                                                    @endphp
                                                    <span>{{ $optionWiseResponses }}</span>
                                                @endif
                                            </div>
                                            <div class="col-lg-12">
                                                <div class="row">
                                                    <div class="col-lg-12">
                                                        <div class="progress">
                                                            @if ($pollModel->show_on_employee == 11)
                                                                <div class="progress-bar bg-teal" style="width: {{$responsePercentage}}%"
                                                                    aria-valuenow="{{$responsePercentage}}" aria-valuemin="0"
                                                                    aria-valuemax="100">{{$responsePercentage}}%</div>
                                                            @else
                                                                <div class="{{ $type }} bg-{{ $color }}"
        ...
        @else
            <div class="d-flex flex-column justify-content-center align-items-center text-center h-100 pt-3">
                ...
                <p class="text-muted pt-3 mb-0">No Poll Found.</p>
            </div>
        @endif
...
<script>
    $(document).ready(function() {
        $('.pollOption[type=radio]').on('change', function() {
            let poll_option_id = $(this).val()
            let poll_id = $('#poll').val()
