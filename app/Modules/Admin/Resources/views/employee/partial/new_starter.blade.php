<div class="card" style="height: 350px;">
    ...
    <div class="card-body p-0">
        @if (count($newEmployeeData) > 0)
            <table class="table text-nowrap">
                <tbody>
                    @foreach($newEmployeeData as $model)
                    <tr>
                        <td>
                            <div class="row">
                                <div class="col-md-8">
                                    <h6 class="font-weight-semibold mb-0">{{ $model->getFullName() }}</h6>
                                    <span class="d-block text-muted"><i class="icon-office font-size-sm mr-1"></i>{{optional($model->department)->title}}</span>
                                    <span class="d-block text-muted"><i class="icon-price-tags font-size-sm mr-1"></i>{{optional($model->designation)->title}}</span>
                                </div>
                                <div class="col-md-4">
                                    <img src="{{ $model->getImage() }}" class="rounded-circle"
                                                        width="40" height="40" alt="">
                                    {{-- <img src="{{ asset('admin/help_desk.png') }}" alt="" class="rounded-circle img-fluid" width="60" height="60"> --}}
                                </div>
                            </div>
                        </td>
                    </tr>
        ...
        @else
            <div class="d-flex flex-column justify-content-center align-items-center text-center h-100">
                ...
                <p class="text-muted pt-3 mb-0">No New Starters.</p>
            </div>
        @endif
    </div>
</div>
