    ...
    <div class="card-header bg-secondary header-elements-inline text-white">
        ...
        <div class="header-elements">
            <div class="list-icons ml-3">
                <span class="badge badge-danger badge-pill">{{ $employeeDepartments->count() }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="card-body table-responsive" style="overflow-x: hidden; padding:0">
        ...
        @if (count($employeeDepartments) > 0)
            ...
            <table class="table text-nowrap">
                <tbody>
                    @foreach ($employeeDepartments as $model)
                        <tr>
                            <td>
                                <div class="d-flex align-items-center">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-muted font-size-sm"><i
                                                class="icon-envelope font-size-sm mr-1"></i>{{ $model->official_email }}
                                            @if ($model->phone)
                                                <div class="text-muted font-size-sm">
                                                    <i class="icon-phone font-size-sm mr-1"></i>{{ $model->phone }}
                                                </div>
                                            @endif
                            ...
                            <td>
                                <h6 class="font-weight-semibold mb-0 text-right">
                                    {{-- <span style="font-size: 20px;" class=" {{ $model->diff_day=="Today"?"text-danger blink_me":'' }}">{{ $model->diff_day }}
                                    </span> --}}
        ...
        @else
            <div class="d-flex flex-column justify-content-center align-items-center text-center h-100 pt-3">
                ...
                <p class="text-muted pt-3 mb-0">No Sub-Function Members Found.</p>
            </div>
        @endif
