@inject('surveyRepo', '\App\Modules\Survey\Repositories\SurveyRepository')
    ...
    <div class="card-header bg-secondary header-elements-inline text-white">
        ...
        <div class="header-elements">
            <div class="list-icons ml-3">
                <span class="badge badge-danger badge-pill">{{ $surveyModels->total() }}</span>
            </div>
        </div>
    </div>

    <div class="card-body" style="overflow-y: scroll">
        @if (count($surveyModels) > 0)
            <ul class="media-list">
                @foreach ($surveyModels as $surveyModel)
                    <li class="media">
                        ...
                        <div class="media-body">
                            <span class="text-secondary">{{ $surveyModel->title }}</span>
                        </div>
                        @php
                            $isSubmitted = $surveyRepo->isResponseSubmitted($surveyModel->id);
                        @endphp

                        @if (isset($isSubmitted) && !empty($isSubmitted))
                            <div class="ml-3 align-self-center mb-3">
                                <a class="btn btn-success btn-sm rounded-pill"
                                    href="{{ route('survey.viewSurveyReportByEmpoyee', $surveyModel->id) }}">View Report</a>
                            </div>
                        @else
                            <div class="ml-3 align-self-center mb-3">
                                <a class="btn btn-success btn-sm rounded-pill"
                                    href="{{ route('survey.viewSurveyByEmpoyee', $surveyModel->id) }}">
                                    Survey
                                </a>
                            </div>
                        @endif
        ...
        @else
            <div class="d-flex flex-column justify-content-center align-items-center text-center h-100 pt-3">
                ...
                <p class="text-muted pt-3 mb-0">No Survey Found.</p>
            </div>
        @endif
