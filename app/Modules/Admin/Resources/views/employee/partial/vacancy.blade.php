    ...
    <div class="card-header bg-secondary header-elements-inline text-white">
        ...
        <div class="header-elements">
            <div class="list-icons ml-3">
                <span class="badge badge-danger badge-pill">{{ $mrfModels->total() }}</span>
            </div>
        </div>
    </div>

    <div class="card-body" style="overflow-y: scroll">
        @if (count($mrfModels)>0)
        <ul class="media-list">
            @foreach($mrfModels as $mrfModel)
                <li class="media">
                    <div class="mr-3">
                        <span class="btn bg-transparent border-success text-success rounded-pill border-2 btn-icon">
                            <i class="icon-megaphone"></i>
                        </span>
                    </div>
                    <div class="media-body">
                        <a href="{{ route('mrf.view', $mrfModel->id) }}"><span class="text-secondary">{{ $mrfModel->title }}</span></a>
                        @if(date('Y-m-d') < $mrfModel->end_date)
                            <div class="text-muted">Deadline :&nbsp;&nbsp;{{ date('M d, Y', strtotime($mrfModel->end_date)) }}</div>
                        @else
                            <div class="text-danger">Deadline :&nbsp;&nbsp;{{ date('M d, Y', strtotime($mrfModel->end_date)) }}</div>
                        @endif
                    </div>
                    @if(date('Y-m-d') < $mrfModel->end_date)
                        <div class="ml-3 align-self-center">
                            <a class="btn btn-success btn-sm rounded-pill" href="{{ route('applicant.create').'?mrf='.$mrfModel->id }}">
                                Apply
                            </a>
                        </div>
                    @endif
        ...
        <div class="d-flex flex-column justify-content-center align-items-center text-center h-100 pt-3">
            ...
            <p class="text-muted pt-3 mb-0">No Vacancy Found.</p>
        </div>
        @endif
