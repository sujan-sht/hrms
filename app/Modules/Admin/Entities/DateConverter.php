...
{    // Data for nepali date
     ...
     * @return string
     */
    public function _get_english_month($m)
    {
        $eMonth = FALSE;
        switch ($m) {
            case 1:
    ...
    public function eng_to_nep($yy, $mm, $dd)
    {
        ...
        if ($chk !== TRUE) {
            ...
            $this->_nep_date['nepali_day'] = $this->_get_day_of_week_nepali($day);
     ...
     * @return string
     */
    public function _get_day_of_week_nepali($day)
    {
        switch ($day) {
            case 1:
                $day = "आइतवार";
    ...
    public function eng_to_nep_convert($date)
    {
        ...
        return $date['year'] . '-' . sprintf("%02d", $date['month']) . '-' . sprintf("%02d", $date['date']);
    }

    public function eng_to_nep_convert_two_digits($date)
    {
        $date = Carbon::parse($date);
        $date = $this->eng_to_nep($date->year, $date->month, $date->day);
        return $date['year'] . '-' . sprintf("%02d", $date['month']) . '-' . sprintf("%02d", $date['date']);
    ...
    public function getNepYears()
    {
        $yearArray = [];
        $firstYear = 2076;
        $current = $this->eng_to_nep(now()->format('Y'), now()->format('m'), now()->format('d'));
        $lastYear = $current['year'];
        // $current = $this->eng_to_nep_convert(date('Y-m-d'));
        // $lastYear = (int) date('Y', strtotime($current));
        for ($i = $firstYear; $i <= $lastYear; $i++) {
            $yearArray[$i] = $i;
        }
        krsort($yearArray);
