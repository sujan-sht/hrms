<table class="table">
    ...
    @if($forapproval_requests->total() != 0)
        @foreach($forapproval_requests as $key=>$value)
            <tr>
                <td scope="row">{{ $value->title }}</td>
                <td>{{ optional($value->requestType)->title  }}</td>
                ...
                <td class="text-teal">
                    <span data-popup="tooltip" data-original-title="{{ $value->status == 1 ? 'Approved' : ($value->status == 2 ? 'Declined' : ($value->status==3 ? 'Forwarded' : 'Pending')) }}" data-placement="bottom">
                        <i class="{{ $value->status == 1 ? 'icon-checkmark4' : 'icon-cross2' }}"></i>
                    </span>
                </td>
                <td>{{optional($value->forwardedBy)->first_name ." ".optional($value->forwardedBy)->middle_name." ".optional($value->forwardedBy)->last_name}}</td>   
                <td colspan="2">{{$value->description}}</td>

                <td>
                    @if(($value->first_approval_id == Auth::user()->id && $value->status == 0) || ($value->forwarded_to == Auth::user()->id && $value->status == 3))
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="actionMenu">
                                <a class="dropdown-item update_request" data-toggle="modal" data-target="#modal_theme_warning"
                                    link="{{route('team-claimRequest.update',$value->id)}}" att-status="{{$value->request_status}}"
...
<span style="margin: 5px;float: right;">
    @if($forapproval_requests->total() != 0)
    {!! $forapproval_requests->appends(\Request::except('page'))->render() !!}
    @endif
</span>