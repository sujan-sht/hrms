    ...
    @if($requests->total() != 0)
        @foreach($requests as $key=>$value)
            <tr>
                ...
                <td class="text-teal">
                    <span data-popup="tooltip" title="{{ $value->status == 1 ? 'Approved' : ($value->status == 2 ? 'Declined' : ($value->status==3 ? 'Forwarded' : 'Pending')) }}" data-placement="bottom">
                        <i class="{{ $value->status == 1 ? 'icon-checkmark4' : 'icon-cross2' }}"></i>
                    </span>
                </td>
                <td>{{optional($value->getApprovedBy)->first_name ." ".optional($value->getApprovedBy)->middle_name." ".optional($value->getApprovedBy)->last_name}}</td>   
