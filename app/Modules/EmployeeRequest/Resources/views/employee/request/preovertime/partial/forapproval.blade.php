    ...
    @if($forapproval_requests->total() != 0)
        @foreach($forapproval_requests as $key=>$value)
            <tr>
                ...
                <td>{{$value->ot_hrs}}</td>
                <td class="text-teal">
                    <span data-popup="tooltip" data-original-title="{{  $value->status }}" data-placement="bottom">
                        <i class="{{ $value->status == 'Approved' ? 'icon-checkmark4' : 'icon-cross2' }}"></i>
                    </span>
                </td>
                ...
                <td>
                    @if(($value->first_approval_id == Auth::user()->id && $value->status == 'Pending') || ($value->forwarded_to == Auth::user()->id && $value->status == 'Forwarded'))
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="actionMenu">
                                <a class="dropdown-item update_request" data-toggle="modal" data-target="#modal_theme_warning"
                                    link="{{route('team-preOvertimeRequest.update',$value->id)}}" att-status="{{$value->status}}"
